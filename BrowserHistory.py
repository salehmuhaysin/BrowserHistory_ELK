...
a_parser = argparse.ArgumentParser('Python script tool to parse web browser history')


requiredargs = a_parser.add_argument_group('required arguments')
requiredargs.add_argument('-i', dest='in_file', help='Input file (History,WebCacheV01.dat,places.sqlite)', required=True)
requiredargs.add_argument('-b', dest='browser', help='browser (IE,firefox,chrome)', required=True)

a_parser.add_argument('-o' , dest='out_file' , help='Output file for all the results')
...
path = args.in_file
if args.browser in ['IE','firefox','chrome']:
...
if browser == "chrome":
	...
	print "[-] Make sure to specify the browser name (-e,-f,-c)"

output_json = json_beautifier(h)
if output is None:
	print output_json
else:
	f = open(output , 'w')
